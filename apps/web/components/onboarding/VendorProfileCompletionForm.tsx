...
export default function VendorProfileCompletionForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
     ...
     if (!businessName.trim() || !phone.trim() || !address.trim() || !description.trim() || (!logoFile && !logoPreview) || (!bannerFile && !bannerPreview)) {
    ...
    let logoImageUrl: string | null = logoPreview && !logoPreview.startsWith('blob:') ? logoPreview : null; // Keep existing URL if any
    let bannerImageUrl: string | null = bannerPreview && !bannerPreview.startsWith('blob:') ? bannerPreview : null; // Keep existing URL if any
      ...
      <div className="bg-white p-8 rounded shadow-md w-full max-w-md text-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
           ...
           <div>
               ...
               <p><strong>Intended Role:</strong> vendor</p> {/* Explicitly show role */}
           </div>
          ...
          <div>
            ...
            {bannerPreview && (
              <div className="relative w-full h-40 mx-auto mb-3 rounded-md overflow-hidden border border-gray-300"> {/* Adjust size */}

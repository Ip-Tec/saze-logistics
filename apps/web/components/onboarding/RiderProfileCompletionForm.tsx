...
export default function RiderProfileCompletionForm() {
  ...
  const [riderImagePreview, setRiderImagePreview] = useState<string | null>(null);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'rider' | 'vehicle') => {
    const file = e.target.files?.[0];
    const setFile = type === 'rider' ? setRiderImageFile : setVehicleImageFile;
    const setPreview = type === 'rider' ? setRiderImagePreview : setVehicleImagePreview;
    const currentPreview = type === 'rider' ? riderImagePreview : vehicleImagePreview;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
     ...
     if (!vehicleType.trim() || !licensePlate.trim() || (!riderImageFile && !riderImagePreview) || (!vehicleImageFile && !vehicleImagePreview)) {
    ...
    let riderImageUrl: string | null = riderImagePreview && !riderImagePreview.startsWith('blob:') ? riderImagePreview : null; // Keep existing URL if any
    let vehicleImageUrl: string | null = vehicleImagePreview && !vehicleImagePreview.startsWith('blob:') ? vehicleImagePreview : null; // Keep existing URL if any
    ...
    if (riderImageFile) {
     ...
     if (!uploadFailed && vehicleImageFile) {
        const result = await uploadVehicleImage(vehicleImageFile, 'vehicle_image'); // Pass file and type
        if (result?.url) {
    ...
    const profileData: ProfileInsert = { // Use Insert type for potential new row
    ...
    try {
        ...
        if (insertError && insertError.code === '23505') { // Unique violation - profile exists
             console.warn("Profile already exists, attempting update instead.");
             const updateData: ProfileUpdate = { // Use Update type
          ...
          <div>
            ...
            {riderImagePreview && (
              <div className="relative w-24 h-24 mx-auto mb-3 rounded-full overflow-hidden border border-gray-300">
                 <img src={riderImagePreview} alt="Rider Preview" className="w-full h-full object-cover" />

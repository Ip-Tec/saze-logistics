...
export default function VendorDashboard() {
  ...
  const [activeMenuItemsCount, setActiveMenuItemsCount] = useState<number | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const totalRev = revenueData?.reduce((sum, order) => sum + (order.total_amount || 0), 0) || 0;
      setTotalRevenue(totalRev);


      // 2. Fetch Active Menu Items Count
      const { count: menuItemsCount, error: menuItemsCountError } = await supabase
          .from('menu_item')
          ...
          chartOrdersData.forEach(order => {
              if (order.created_at) {
                  ...
                  if (order.status === 'delivered' && order.total_amount !== null) {
                      dailyRevenue[formattedDate] = (dailyRevenue[formattedDate] || 0) + order.total_amount;
                  }
              }
          });
      }
      ...
      const revenueChart: ChartDataPoint[] = Object.keys(dailyRevenue).map(date => ({
          date: date,
          value: dailyRevenue[date],
      })).sort((a, b) => new Date(a.date) as any - (new Date(b.date) as any)); // Sort by date


       const ordersChart: ChartDataPoint[] = Object.keys(dailyOrders).map(date => ({
          date: date,
  ...
  return (
    <div className="p-6 flex flex-col gap-4 w-full h-full overflow-y-auto glass-scrollbar"> {/* Added overflow and scrollbar */}
      {/* Cards */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-2xl bg-white/10 p-5 backdrop-blur border border-white/20 shadow-md">
          ...
          <p className="text-2xl font-semibold text-white">{totalOrders !== null ? totalOrders : 'N/A'}</p>
        </div>
        <div className="rounded-2xl bg-white/10 p-5 backdrop-blur border border-white/20 shadow-md">
          ...
          <p className="text-2xl font-semibold text-white">
            ₦{totalRevenue !== null ? totalRevenue.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : 'N/A'}
          </p>
        </div>
        <div className="rounded-2xl bg-white/10 p-5 backdrop-blur border border-white/20 shadow-md">
          <h2 className="text-sm text-gray-50 mb-2">Active Menu Items</h2>
           {/* Display fetched activeMenuItemsCount, handle null state */}
          <p className="text-2xl font-semibold text-white">{activeMenuItemsCount !== null ? activeMenuItemsCount : 'N/A'}</p>
        </div>
      </div>

      {/* Chart Section with Toggle */}
      <div className="w-full rounded-2xl bg-white/10 p-6 backdrop-blur border border-white/20 shadow-md flex-1 min-h-0"> {/* Added flex-1 min-h-0 */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-white font-semibold">
            {activeChart === "revenue" ? "Revenue Chart (Last 30 Days)" : "Orders Chart (Last 30 Days)"} {/* Added timeframe */}
          </h2>
        ...
        {(activeChart === 'revenue' && revenueChartData.length === 0) || (activeChart === 'orders' && ordersChartData.length === 0) ? (
           ...
           <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <Line
                ...
                strokeWidth={3}
                 dot={{ r: 4 }} // Add dots
                 activeDot={{ r: 8, stroke: activeChart === "revenue" ? "#4ADE80" : "#FF6900", fill: '#fff' }} // Larger dot on hover
              />
            </LineChart>
          </ResponsiveContainer>
        )}
       ...
       <div className="w-full"> <OredrsList /> </div>
    </div>
  );
}
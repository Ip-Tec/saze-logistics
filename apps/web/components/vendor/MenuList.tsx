...
export default function MenuList({ vendorId, onEditMenuItem }: MenuListProps) { // Receive onEditMenuItem prop
          ...
          {itemsByCategory[category.id] && itemsByCategory[category.id].length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {itemsByCategory[category.id].map((item) => (
                ...
                >
                   <div className="flex items-center gap-4"> {/* Wrap image and text content */}
                     ...
                     {item.imageUrl ? ( // Use the processed imageUrl
                         <img
                             ...
                             onError={(e) => { // Handle broken image links
                                 ...
                                 e.currentTarget.src = 'https://placehold.co/64x64?text=No+Image'; // Placeholder
                             }}
                         />
                     ) : (
                    ...
                    <div className="flex-1"> {/* Use flex-1 to make text content fill space */}
                      <h3 className="text-black text-base font-medium">{item.name}</h3>
                       {item.description && (
                           <p className="text-gray-700 text-sm mt-1">{item.description}</p>
                       )}
                      <p className="text-green-700 font-semibold mt-2">
                        ₦{item.price?.toFixed(2) || '0.00'} {/* Format price, handle null */}
                      </p>
                    </div>
                   </div>

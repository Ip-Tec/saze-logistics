  ...
  const handleMenuItemSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        ...
        const success = await onUpdateMenuItem(
          updatePayload,
          itemImageFiles,
          imageIdsToDelete
        );
        ...
        if (!addedItem || !addedItem.id) {
          throw new Error(
            "Failed to retrieve new menu item ID after creation."
          );
        }
          ...
          for (const file of itemImageFiles) {
            ...
            const { data: uploadData, error: uploadError } =
              await supabase.storage
                .from("vendorassets") // Replace with your Supabase Storage bucket name
            ...
            if (uploadError) {
              ...
              if (imageUrl) {
                ...
                const { data: imageData, error: imageInsertError } =
                  await supabase.from("menu_item_image").insert([
                    {
      ...
      toast.error(
        `Failed to submit menu item: ${error.message || "Unknown error"}`
      );
    } finally {
      setIsSubmitting(false);
    }
                  ...
                  {existingImages.map((img) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleRemoveExistingImage(img.id, img.image_url)
                        }
